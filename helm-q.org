...
=helm-q= manages the Q-Shell buffer name via variable =helm-q-qcon-buffer-name-pattern=, which is ='("." (service env region))= by default.
The first item in the list is the concatenator to join the fields in second item.
These fields in second item can be found in file `instances-meta.json'.

For example, if =instances-meta.json= contains instance with attributes:
#+begin_example
service:"TAQ.HDB"
env:"prod"
region:"nam"
...
- a prefix =*qcon-=,
- the string joining fields =service=, =env=, =region= with a dot(=.=), and
- a suffix =*=.
* implementation
** helm-q buffer class
